class ZomatoChatBot {
  ...
  async initialize() {
    try {
      ...
      console.error('Zomato chat bot initialization failed:', error);
    }
  }
  ...
  async waitForChatInterface() {
    ...
    console.log('Zomato chat interface not found');
  }
  ...
  findChatElements() {
    ...
    if (!this.chatContainer) {
      this.chatContainer = this.messageInput.closest('.chat-section') || 
  ...
  isFromSupport(messageElement) {
    ...
    const hasUserClass = classList.includes('user') || 
  ...
  async typeAndSendResponse(responseText) {
    ...
    if (this.sendButton && !this.sendButton.disabled) {
      ...
      this.messageInput.dispatchEvent(enterEvent);
    }
  }
...
if (window.location.hostname.includes('zomato.com')) {
  const zomatoChatBot = new ZomatoChatBot();
}